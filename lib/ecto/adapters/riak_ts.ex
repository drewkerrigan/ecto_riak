defmodule Ecto.Adapters.RiakTS do
  ...
  defmacro __before_compile__(_opts), do: :ok # TODO: Get config from here for Riak app
  ...
  def child_spec(_repo, _options) do
    {:riak, {Riak.Connection, :start_link, ['127.0.0.1', 8087, []]},
     :transient, 5000, :supervisor, [Riak.Connection]}
  end
  ...
  def execute(repo,
        ...
        {:nocache, {:all, query}}, [], process, options) do
    case Riak.Timeseries.query(all(query)) do
      {_fields, []} -> {0, []};
      {:error, {_, message}} ->
        raise Error, message;
      {_fields, raw_rows} ->
  ...
  def execute(repo,
        %{fields: fields,
          sources: {{table, schema}}},
        {:nocache, {:all, query}}, params, process, options) do
  ...
  def insert_all(_repo, %{source: {_, table}, schema: schema},
        header, rows, returning, _options) do
    fields = schema.__schema__(:fields)
  ...
  def delete(_repo, %{source: {_, table}, schema: schema}, filter, _opts) do
    pk = schema.__schema__(:primary_key)
